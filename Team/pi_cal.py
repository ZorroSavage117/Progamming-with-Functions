...
def populate_main_window(frm_main):
    ...
    lbl_age = Label(frm_main, text="Radius of the Circle")
    ...
    lbl_slow = Label(frm_main)
    # lbl_fast = Label(frm_main, width=3)
    # lbl_rate_units = Label(frm_main, text="beats/minute")

    # Create the Clear & Calculate button.
    cal_clear = Button(frm_main, text="Cal")
    ...
    cal_clear.grid(row=2, column=0, padx=3, pady=3, columnspan=2, sticky="w")
    btn_clear.grid(row=2, column=1, padx=3, pady=3, columnspan=2, sticky="w")
    ...
    cal_clear.config(command=calculate)
